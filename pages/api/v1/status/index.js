...
async function status(request, response) {
  ...
  const dabatase_max_connections_value =
    await database_max_connections_result.rows[0].max_connections;

  const database_max_connections_parsed_value = parseInt(
    dabatase_max_connections_value,
  );
  ...
  const database_open_connections_value =
    database_open_connections_result.rows[0].count;
